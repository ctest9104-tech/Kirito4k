export default async function handler(req, res) {
  ...
  // We try three different sources in order of "Ad-Free-ness"
  const endpoints = [
    episode 
  ...
  // If all direct links fail, we send back a 404 to tell the app to use the "Safe Player"
  return res.status(404).json({ error: "Direct stream blocked." });
}
